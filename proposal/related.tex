% !TEX root = proposal.tex
 and present my previous research
...
\subsection{Evaluation for Previous Systems}
...
LIFT~\cite{lift:micro2006}(DFT (NO\_FP)). The last column(in-line DFT) shows
the result for DFT implementation that inlines the analysis to the application
process~\cite{tfa:ndss2012}. CPU usage is partitioned to show the amount of CPU
cycles taken from the application and analysis threads separately. The darker
