...
% libdft
We implemented \libdft~\cite{libdft:2012vee} which is highly
optimized/efficient DFT system which runs for binary program executed from
commodity hardware system.  \libdft supports generic API that helps anyone to
customize and build tools that serve for different problem domains. 
%
...
The immediate follow-up work for \libdft is a work~\cite{tfa:2012ndss} which
decouples tracking logics from the original program logics and represent
tracking logic as as a specially designed intermediate representation~(IR) of
\TFA.  Number of optimizations for compiler and DFT specific ones are applied
to \TFA. Then the optimization result is injected back to the original program. 
%
...
without compromising the correctness~\footnote{In this context, we {\it
 correctness} mean that the same level of security guarantees as the
baseline tool which is \libdft} of DFT operations.
