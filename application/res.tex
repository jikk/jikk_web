...
\TFA) for the equivalent analysis.
%
...
that serves for enhancement of software system reliability and I have
...
%
For DFT implementation and most of similar security systems that in-line
...
information and transfer it to another core which would eventually replace the
...
project is to make additional hardware minimal and flexible so as it can
support other kinds of analysis with simple modification of software component.
...
instrumentation frameworks each supports source code and binary respectively.
%
This hybrid approach introduces number of interesting research challenges such
as how to maintain a single execution unit integrating two very different
...
environments(which do not have source access), we can use VM-based
...
%
We are planning to apply the methodology to evaluate number of different DFT
...
aspects of three frameworks. As their behavioral and performance implications
...
choice when they need to find a instrumentation framework that meets for their
need.
...
operation that triggers an integer error. The other example is a pair of the
location of integers errors flows into security sensitive places such as
arguments for memory allocation functions~({\tt malloc}, {\tt strcpy}, etc.).
%
...
invented number of novel optimization techniques and frameworks composed of
static and dynamic components that establish feedback loop.
%
Besides DFT, I am also interested in system security topics related to
hardening software systems and make those immune to exploitable defects or
unintended bugs. 
%
